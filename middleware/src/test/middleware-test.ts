...
test('throws if no proxyUrl given', async t => {
    const makeMiddlewareUntyped = rendertron.makeMiddleware as (options?: unknown) => express.Application;
...
test('forwards proxy error status and body', async t => {
    ...
    const proxyUrl = await listen(express().use((_req, res) => res.status(500).end('proxy error')));
    const appUrl = await listen(makeApp({ proxyUrl }));
