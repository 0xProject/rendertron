...
export class DatastoreCache {
    ...
    async clearCache() {
        ...
        const entityKeys = entities.map(entity => (entity as DatastoreObject)[this.datastore.KEY]);
    ...
    middleware() {
        ...
        return async function(this: DatastoreCache, ctx: Koa.Context, next: () => Promise<unknown>) {
